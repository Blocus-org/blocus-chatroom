<?php
  ...
  session_start();
  $version = "alpha-1.1.4";
  define("CONFIGPATH", "php/config.php");
  $test = file_get_contents(CONFIGPATH);
  ...
  if (!$test) {
    ...
    if (file_put_contents(CONFIGPATH, $filecontent)) {
      file_put_contents(CONFIGPATH, $filecontent_backup);
      ...
      $php_error = " ☒ Checked write permission on config.php: --------
        <span style='color:Red;'>Cannot write config file... Please check files permissions
         (read and write permission for wwww-data on full website)</span><br>";
      echo $php_error;
    }
  }
